...
def default_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ...
      max_motor_velocity=3.50, # 4.82 max without load
      ...
      reward_config=config_dict.create(
          scales=config_dict.create(
              ...
              feet_clearance=-0.5, # -> was -0.5
              feet_air_time=3.0, # was 2.0 
              feet_slip=-0.25, # was -0.25
              feet_drag=-1.5, 
              feet_height=0.0,
              feet_phase=3.0, # was 1.0
              # Other rewards.
              stand_still=-0.3, # penalize when command = 0
              alive=0.0,
              termination=-2.0,
      ...
      push_config=config_dict.create(
        ...
        lin_vel_x=[-0.4, 0.4], # TODO : train with this
        lin_vel_y=[-0.2, 0.2],
        ang_vel_yaw=[-1.0, 1.0],
  )
...
class Joystick(bd5_base.BD5Env):
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        rng, key = jax.random.split(rng)
        gait_freq = jax.random.uniform(key, (1,), minval=0.8, maxval=1.2)
